...
pub trait ShellProvider: Send + Sync + 'static {
    ...
    // Window shell controls, for documents that draw their own titlebar
    ...
    /// Himalayas patch: set the document's page-content zoom level (`1.0`
    /// is unzoomed), for embedder UI that draws its own zoom control (e.g.
    /// a settings-panel slider) and therefore only has a `ShellProvider`
    /// handle, not direct access to the window's `Viewport`. Proxies to
    /// the same `Viewport::set_zoom` the built-in Cmd/Ctrl+=/-/0 keyboard
    /// shortcuts already use. No-op by default (matches every other
    /// optional method on this trait).
...
impl Default for Viewport {
    fn default() -> Self {
        Self {
            window_size: (0, 0),
            hidpi_scale: 1.0,
            // Himalayas patch history: tried raising this default (1.3, then
            // 1.5) per "start more zoomed in" feedback — reverted back to
            // real 100% once the actual ask turned out to be page content
            // at 100% with only the *browser shell* (address bar/tabs)
            // zoomed in. `Viewport::zoom` can't express that split — it's
            // one value inherited by sub-documents (loaded pages) from the
            // outer shell document every frame (`resolve.rs`'s subdoc
            // loop), so any non-1.0 value here zooms both together. The
            // shell-only sizing now lives as literal font-size/dimension
            // values in `src/bin/desktop.rs`'s RSX instead — see that
            // file's design-tokens comment.
            zoom: 1.0,
