...
use blitz_dom::{BaseDocument, Document, PlainDocument, Widget};
...
impl SubDocumentAttr {
    pub fn new(doc: BaseDocument) -> Self {
        Self::from_document(PlainDocument(doc))
    }

    /// Like [`SubDocumentAttr::new`], but for a caller-supplied [`Document`]
    /// implementation instead of the plain, non-scriptable `PlainDocument`
    /// wrapper — e.g. a document that overrides `handle_ui_event` to run
    /// page-authored JavaScript. See docs/NATIVE_RENDERING_PLAN.md in the
    /// Himalayas Browser repo for why this exists: `PlainDocument::handle_ui_event`
    /// always uses `NoopEventHandler`, so there is no way to get a `<web-view>`
    /// with scripting support through `SubDocumentAttr::new` alone.
    pub fn from_document<D: Document + 'static>(doc: D) -> Self {
        let id = doc.id();
