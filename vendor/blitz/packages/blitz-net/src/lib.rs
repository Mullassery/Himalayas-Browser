...
// Only meaningful with `--features cache` (`cargo test -p blitz-net
// --features cache`) — without it, `Provider` falls back to a plain
// `reqwest::Client` with no caching at all, so `second_fetch_of_a_cacheable_url_does_not_hit_the_network_again`
// would (correctly) fail. Not run by the default outer `cargo test`, which
// doesn't turn this feature on for the `himalayas` crate's own test runs
// (only `js_engine` enables it) — see `docs/NATIVE_RENDERING_PLAN.md`'s
// caching section for why the feature exists but was dormant until now.
...
mod cache_tests {
    ...
    /// Spawns a minimal local HTTP/1.1 server on an ephemeral port that
    /// answers every request with a 200 carrying `Cache-Control:
    /// max-age=3600` (safely cacheable, no revalidation needed) and
    /// increments `hits` once per accepted connection. Returns the bound
    /// address. No real HTTP crate needed — one fixed canned response is
    /// all a cache-hit-count test requires.
    async fn spawn_cacheable_server(hits: Arc<AtomicUsize>) -> std::net::SocketAddr {
        ...
        tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    // Just drain whatever the client sent; we don't parse it,
                    // every request gets the same canned cacheable response.
    ...
    #[tokio::test]
    async fn second_fetch_of_a_cacheable_url_does_not_hit_the_network_again() {
        let hits = Arc::new(AtomicUsize::new(0));
        let addr = spawn_cacheable_server(hits.clone()).await;
        let url: blitz_traits::net::Url =
            format!("http://{addr}/resource").parse().unwrap();

        let provider = Provider::new(None);
        // Clean slate: the disk cache is keyed by URL and lives in a fixed,
        // real OS cache directory shared across every `Provider` instance
        // and every test run (see `get_cache_path`), so a stale entry from
        // a previous run could make this pass for the wrong reason (or
        // fail confusingly) without this.
        provider.clear_cache().await;

        let (_, first_bytes) = provider
            .fetch_async(blitz_traits::net::Request::get(url.clone()))
            .await
            .expect("first fetch should succeed");
        let (_, second_bytes) = provider
            .fetch_async(blitz_traits::net::Request::get(url))
            .await
        ...
        assert_eq!(
            ...
            1,
            "second fetch of a cacheable URL should be served from cache, not hit the network again"
        );
