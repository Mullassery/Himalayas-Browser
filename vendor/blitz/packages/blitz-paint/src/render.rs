...
impl<'dom, 'a> BlitzDomPainter<'dom, 'a> {
    ...
    pub fn paint_scene(&self, scene: &mut impl PaintScene) {
        ...
        // Himalayas patch: overlay scrollbar for ordinary whole-page
        // scrolling — see `draw_viewport_scrollbars` and
        // `BaseDocument::viewport_wants_scrollbar` for why this needed its
        // own path (upstream's overlay scrollbars only ever covered a
        // node's own `overflow: auto/scroll`, never the viewport itself).
        // Drawn here, outside `render_element`'s scrolled transform, so it
        // stays pinned to the window edge rather than scrolling with the
        // page — matching how a real overlay scrollbar behaves.
        #[cfg(feature = "scrollbars")]
        self.draw_viewport_scrollbars(scene);
    ...
    /// Himalayas patch: overlay scrollbar for ordinary whole-page scrolling
    /// — see `BaseDocument::viewport_wants_scrollbar`/`viewport_scrollbar_thumb`
    /// for what this covers that `ElementCx::draw_scrollbars` (a node's own
    /// `overflow: auto/scroll`) doesn't. No hover/drag-to-scroll
    /// interactivity here (unlike `draw_scrollbars`) — that needs threading
    /// viewport-scrollbar hit-testing through the same
    /// `hovered_scrollbar`/`scrollbar_drag_target` machinery `ScrollbarRef`
    /// already has for per-node scrollbars, a separate follow-up; this is
    /// the passive "shows while/just after scrolling, fades out" half,
    /// which is what was actually missing (real users can't drag-scroll a
    /// touchpad-first pointer anyway).
    #[cfg(feature = "scrollbars")]
    fn draw_viewport_scrollbars(&self, scene: &mut impl PaintScene) {
        use taffy::AbsoluteAxis;

        let Some(root_id) = self.root_element_id else { return };
        let opacity = self.dom.scrollbar_opacity(root_id);
        if opacity == 0.0 {
            return;
        }

        let dark_scheme =
            self.dom.viewport().color_scheme == blitz_traits::shell::ColorScheme::Dark;
        // Darker/more opaque than the per-node overlay scrollbar colors —
        // "bigger and more prominent" feedback applied to color as well as
        // size, since this bar is always visible (not just during/after a
        // scroll) and needs to read clearly against any page background.
        let (track_color, thumb_color, stroke_color) = if dark_scheme {
            (
                Color::from_rgba8(0, 0, 0, 60),
                Color::from_rgba8(224, 224, 224, 230),
                Color::from_rgba8(0, 0, 0, 140),
            )
        } else {
            (
                Color::from_rgba8(0, 0, 0, 30),
                Color::from_rgba8(90, 90, 90, 230),
                Color::from_rgba8(255, 255, 255, 140),
            )
        };

        let base_transform = Affine::translate(Vec2 {
            x: self.initial_x * self.scale,
            y: self.initial_y * self.scale,
        });

        for axis in [AbsoluteAxis::Vertical, AbsoluteAxis::Horizontal] {
            if !self.dom.viewport_wants_scrollbar(axis) {
                continue;
            }
            let Some(thumb) = self.dom.viewport_scrollbar_thumb(axis) else {
                continue;
            };
            ...
            // Full-length track behind the thumb, so the whole scrollbar
            // (not just the thumb) reads as a persistent, prominent UI
            // element rather than a thin floating pill.
            let (window_width, window_height) = self.dom.viewport().window_size;
            let full_track = match axis {
                AbsoluteAxis::Vertical => Rect::new(rect.x0, 0.0, rect.x1, window_height as f64),
                AbsoluteAxis::Horizontal => Rect::new(0.0, rect.y0, window_width as f64, rect.y1),
            };
            scene.fill(
                Fill::NonZero,
                base_transform,
                ...
                None,
                &full_track.to_rounded_rect(radius),
            );

            scene.fill(
                Fill::NonZero,
                base_transform,
                thumb_color.multiply_alpha(opacity),
                None,
