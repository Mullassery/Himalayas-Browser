...
pub enum BlitzShellEvent {
    ...
    /// Himalayas patch: set the document zoom level programmatically (e.g.
    /// a page-zoom control in an embedder-drawn settings panel), the same
    /// underlying `Viewport::zoom` the existing Cmd/Ctrl+=/-/0 keyboard
    /// shortcuts already drive (`window.rs`'s `KeyboardInput` handler) —
    /// this just gives embedder UI code (which only has a `ShellProvider`,
    /// not direct access to the window's `doc`) a way to reach it too. See
    /// `ShellProvider::set_zoom` (`blitz-traits/src/shell.rs`).
    SetZoom {
        window_id: WindowId,
        zoom: f32,
    },
