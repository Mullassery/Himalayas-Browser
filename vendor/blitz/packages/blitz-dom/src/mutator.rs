...
impl<'doc> DocumentMutator<'doc> {
    ...
    fn load_image(&mut self, target_id: NodeId) {
        // Real `loading="lazy"` support: defer the actual fetch until the
        // image is near the viewport, rather than fetching every `<img>` on
        // the page eagerly at parse/attribute-set time regardless of
        // position — see `BaseDocument::check_lazy_images`
        // (called from `resolve()`) for what "near" means and when this
        // gets revisited. Distinct from — and composes with — the
        // data-attribute lazy-load *convention* fallback below
        // (`data-src`/`data-original`/etc.): that's about finding the real
        // URL when JS-driven lazy-load libraries hide it behind a
        // placeholder `src`; this is about the *standard* `loading`
        // attribute controlling *when* any URL (real or already-resolved
        // via that fallback) actually gets fetched.
        let node = &self.doc.nodes[target_id];
        let wants_lazy_load = node
            .attr(local_name!("loading"))
            .is_some_and(|v| v.eq_ignore_ascii_case("lazy"));
        if wants_lazy_load {
            self.doc.lazy_images.insert(target_id);
            return;
        }
