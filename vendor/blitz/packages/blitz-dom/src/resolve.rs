...
impl BaseDocument {
    ...
    pub fn resolve(&mut self, current_time_for_animations: f64) {
        ...
        // `<img loading="lazy">` — now that layout/transforms are current,
        // check whether any deferred images have scrolled near enough to
        // the viewport to start fetching. No-ops immediately once
        // `lazy_images` is empty (the common case: pages with no lazy
        ...
        for &node_id in &self.sub_document_nodes {
            ...
            if let Some(sub_doc_trait_obj) = node.subdoc_mut() {
                // Himalayas patch: give sub-documents (e.g. a scripted
                // `<web-view>` page) a real per-frame hook via the
                // `Document::poll` trait method, which already exists for
                // exactly this ("poll any pending async operations... flush
                // changes") but upstream is only ever called on the
                // top-level document (see `WindowState::poll` in
                // blitz-shell) — sub-documents never got a tick at all. This
                // is what `requestAnimationFrame` is built on (see
                // `ScriptedDocument::poll` in Himalayas' src/bin/desktop.rs):
                // returning `true` here (because a callback ran and might
                // have queued another) is OR'd into `subdoc_is_animating`
                // below, which keeps the redraw loop alive frame-to-frame
                // for exactly as long as script keeps requesting frames —
                // same mechanism that already drives CSS animations.
                let subdoc_wants_more_frames = sub_doc_trait_obj.poll(None);
                let mut sub_doc = sub_doc_trait_obj.inner_mut();
                ...
                subdoc_is_animating |= sub_doc.is_animating() || subdoc_wants_more_frames;
            }
        }
    ...
    pub fn resolve_scroll_animation(&mut self) {
        match &mut self.scroll_animation {
            ScrollAnimationState::Fling(fling_state) => {
                ...
                self.scroll_by(fling_state.target, dx, dy, &mut |_| {});
    ...
    /// Himalayas patch: hands wheel-scroll momentum off to the existing
    /// `Fling` animation once wheel input has gone quiet for
    /// `WHEEL_MOMENTUM_GRACE_MS` — see `WheelMomentumState`/`handle_wheel`
    /// in `events/pointer.rs` for where samples get collected. Called every
    /// frame (like `resolve_scroll_animation`, right before it), so this is
    /// also what makes `is_animating()` need to know about `wheel_momentum`
    /// — without a live animation already running, nothing would otherwise
    /// keep `resolve()` ticking to notice the grace period elapsed.
    pub fn resolve_wheel_momentum(&mut self) {
        if self.scroll_animation != ScrollAnimationState::None {
            // A fling (from this or a previous gesture) is already
        ...
        let time_ms = SystemTime::now()
            ...
            .unwrap()
            .as_millis() as u64;
        if time_ms - last.time < crate::events::WHEEL_MOMENTUM_GRACE_MS {
            return; // still within a gesture, or another frame will catch it
        }

        if let Some((x_velocity, y_velocity)) = state.velocity() {
            if x_velocity.abs() >= 0.1 || y_velocity.abs() >= 0.1 {
                self.scroll_animation = ScrollAnimationState::Fling(crate::events::FlingState {
                    target: state.target,
