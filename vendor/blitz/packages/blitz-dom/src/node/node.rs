...
/// Same shape as `universal_accessors!`, except the "this node kind doesn't
/// carry that field" case returns a per-thread throwaway scratch value
/// instead of panicking. Used for the fields taffy's layout algorithms
/// (`LayoutPartialTree`/`LayoutBlockContainer`/`CacheTree` in
/// `layout/mod.rs`) read/write on every node in a container's
/// `layout_children` — which, on real-world pages, is not a guarantee that
/// every such node is actually an Element/AnonymousBlock/Document.
/// Confirmed live: a real thehindu.com page crashed the entire renderer via
/// exactly this (a Text node reaching `get_block_child_style` un-wrapped —
/// see docs/NATIVE_RENDERING_PLAN.md for what's understood about why and
/// why the anonymous-box-generation root cause in `layout/construct.rs`
/// isn't chased further here). Reads/writes to the scratch value for a
/// phantom node are inherently meaningless (there's no real box being
/// tracked), so sharing one scratch instance per field per thread across
/// every such node — rather than one per node — is fine: nothing ever
/// depends on it holding a particular node's data.
macro_rules! graceful_layout_accessors {
    ($($(#[$meta:meta])* $field:ident / $field_mut:ident : $ty:ty = $default:expr),* $(,)?) => {
        impl Node {
            $(
                $(#[$meta])*
                #[inline]
                pub fn $field(&self) -> &$ty {
                    thread_local! {
                        static SCRATCH: &'static $ty = Box::leak(Box::new($default));
                    }
                    match &self.data {
                        NodeData::Element(data) | NodeData::AnonymousBlock(data) => &data.$field,
                        NodeData::Document(data) => &data.$field,
                ...
                pub fn $field_mut(&mut self) -> &mut $ty {
                    thread_local! {
                        static SCRATCH: Cell<*mut $ty> = Cell::new(Box::into_raw(Box::new($default)));
                    }
                    match &mut self.data {
                        NodeData::Element(data) | NodeData::AnonymousBlock(data) => &mut data.$field,
                        NodeData::Document(data) => &mut data.$field,
                        // SAFETY: leaked once per thread, never freed, never
                        // aliased concurrently (this whole pipeline is
                        // single-threaded per document resolve pass) — see
                        // the macro's doc comment for why sharing one
                        // instance across every phantom node is sound.
                        _ => SCRATCH.with(|s| unsafe { &mut *s.get() }),
                    }
                }
            )*
        }
...
graceful_layout_accessors! {
    cache / cache_mut: Cache = Cache::default(),
    unrounded_layout / unrounded_layout_mut: Layout = Layout::default(),
    final_layout / final_layout_mut: Layout = Layout::default(),
    scroll_offset / scroll_offset_mut: crate::Point<f64> = crate::Point { x: 0.0, y: 0.0 },
    scrollable_overflow / scrollable_overflow_mut: KurboRect = KurboRect::ZERO,
    transform / transform_mut: Option<Affine> = None,
    display_constructed_as / display_constructed_as_mut: StyloDisplay = StyloDisplay::inline(),
}

thread_local! {
    /// Fallback for `Node::style()` on a node kind that doesn't carry a real
    /// `Style` — see that method. Thread-local rather than a plain `static`
    /// because `Style<Atom>` isn't `Sync` (its compact internal
    /// representation stores lengths as a `*const ()`-sized tagged union,
    /// which rustc conservatively refuses to let cross a thread boundary
    /// even though this particular value — the all-defaults `DEFAULT` const,
    /// never mutated — has no actual pointee to race on). Leaked once per
    /// thread on first use, not per call.
    static DEFAULT_STYLE: &'static Style<Atom> = Box::leak(Box::new(Style::DEFAULT));
}

impl Node {
    /// Layout style for this node — must never panic, unlike the other
    /// `universal_accessors!`-generated fields it used to be defined
    /// alongside (pulled out into its own impl for exactly this reason).
    /// `taffy`'s block layout (`LayoutPartialTree::get_core_container_style`
    /// / `LayoutBlockContainer::get_block_child_style`, both in
    /// `layout/mod.rs`) calls this on whatever nodes end up in a
    /// container's `layout_children` — reachable with a non-Element/
    /// AnonymousBlock/Document node kind on real-world pages (confirmed
    /// live: crashed the entire renderer loading thehindu.com, not a
    /// synthetic edge case — some child ended up in a block container's
    /// layout children without being normalized into an anonymous block
    /// first). A crashed renderer is a strictly worse outcome than
    /// rendering that one node with default/initial styles, so this
    /// degrades instead of panicking — the same principle already applied
    /// to `apple.com`'s blank-render case and the SVG NaN-path paint bug
    /// (see docs/NATIVE_RENDERING_PLAN.md): contain and keep going rather
    /// than crash the whole page over one node.
    #[inline]
    pub fn style(&self) -> &Style<Atom> {
        match &self.data {
            NodeData::Element(data) | NodeData::AnonymousBlock(data) => &data.style,
            NodeData::Document(data) => &data.style,
            _ => DEFAULT_STYLE.with(|s| *s),
        }
    }

    /// Unlike `style()`, this one *is* still safe to leave panicking:
    /// mutable access is only ever taken during style resolution/cascade
    /// (`stylo.rs`), which only ever visits nodes it has already confirmed
    /// are elements — not from layout's node-kind-agnostic child iteration,
    /// which is what made the read path unsafe to leave panicking.
    #[inline]
    pub fn style_mut(&mut self) -> &mut Style<Atom> {
        match &mut self.data {
            NodeData::Element(data) | NodeData::AnonymousBlock(data) => &mut data.style,
            NodeData::Document(data) => &mut data.style,
            _ => panic!("`style_mut` is not available on this node kind"),
        }
    }
}
