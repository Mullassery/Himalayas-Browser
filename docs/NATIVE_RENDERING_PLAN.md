...
8. **Animated images** (GIF/WebP/APNG): `ImageHandler::parse` only ever calls `.decode()`, never `AnimationDecoder`/`into_frames` — every image renders as one static frame forever. Needs a per-image frame list plus an animation clock/repaint scheduler. Natural first step before audio/video since it's scoped to the *existing* image pipeline, not a wholly new element type.
9. **`<audio>` + basic HTML5 playback**: new `SpecialElementData::Audio`, real decode (start with MP3/AAC/Opus/WAV — the "core priority" set from the original ask) via a real Rust audio decode+output crate, play/pause/seek/volume/mute controls wired to real JS Media element semantics. Deliberately not in this first slice: Web Audio API (`AudioContext`/audio graphs), WebRTC audio, Media Session API, multi-device output routing — each a separate, sizable follow-up.
10. **`<video>` + basic HTML5 playback**: new `SpecialElementData::Video`, reusing Phase 3's audio decode/output work for the audio track, a real video frame decoder (start with H.264 — broadest real-world coverage) feeding the existing GPU paint pipeline as a texture per frame, `poster` attribute support (trivial — reuses the existing image-decode pipeline directly), play/pause/seek/volume/fullscreen. Deliberately not in this first slice: adaptive streaming (HLS/DASH), hardware-accelerated decode, DRM/EME, live streaming, WebRTC, WebCodecs, subtitles/WebVTT, multi-track audio/subtitle selection — each large enough to be its own phase, sequenced after a real baseline `<video>` exists to build them on top of.
