...
### Image decoder resource limits (Phase 4.13)

First (and, for this pass, only) item picked up from the "speculative/revisit later" queue — the rest of it (adaptive media optimization, display-configuration modes, HDR/WebGL/AI-CV pipeline work, context-menu expansion) either genuinely depends on `<video>` landing first (not built — see Phase 3's video write-up) or needs its own fresh scoping pass on a vision doc rather than an assumption. This one was concretely actionable on its own: real, scoped security hardening, no dependency on anything else in the queue.

**Correcting an assumption before writing any code**: went in expecting `ImageHandler::parse`'s `image::ImageReader::new(...).decode()` call had *zero* size/allocation bounds — checking the actual `image` crate source first (not assuming) found it already applies a default `Limits` automatically (`max_alloc: 512MB`, no dimension cap) via `ImageReader::new`. So this wasn't "adding protection where there was none," it was tightening a real-but-generous existing default — 512MB is a lot for a page that can reference many images at once (a real multi-image resource-exhaustion angle even when no single decode exceeds the crate's own cap). Tightened to `max_image_width`/`max_image_height: 16384` and `max_alloc: 128MB` per image (`image_decode_limits()`, `net.rs`) — generous for any real photo/graphic (16384×16384 RGBA8 alone is already 1GB uncompressed, which `max_alloc` independently catches too — the two limits back each other up) while meaningfully capping worst-case per-image cost.

**A real, actually-unprotected gap, found by checking rather than assuming again**: the animated-GIF path (`decode_animated_gif`, added in Phase 3.8) builds `GifDecoder` directly rather than through `ImageReader` — meaning it never benefited from the crate's default limits *at all*, not even the generous 512MB one. Fixed via `GifDecoder`'s own `ImageDecoder::set_limits` (needed importing the `ImageDecoder` trait to bring the method into scope), applying the same `image_decode_limits()`. Also added `MAX_ANIMATED_FRAMES` (10,000) — a real, separate gap `Limits` itself doesn't cover at all (per-frame dimensions/allocation, not frame *count*): a GIF with an unreasonable number of individually-tiny frames would pass every per-frame check while still being unreasonable to decode and hold in memory (10,000 frames at the existing 100ms-floor delay is already over 16 minutes of animation — far beyond any real site's actual GIF).

Caught by the new tests, not inspection: the frame-count-cap test (encoding 10,001 frames to prove the cap actually fires one past the boundary, not off-by-one in either direction) hit a real, pre-existing latent bug in the *test helper* itself (`encode_test_gif`, from Phase 3.8) — its per-frame color calculation, `(i as u8 + 1) * 50`, overflows `u8` for any `i` past a handful of frames, previously invisible because every existing caller only ever encoded 1–3 frames. Fixed by cycling the test color every 5 frames (`(i % 5) as u8 + 1) * 50`) — safe regardless of frame count, and doesn't change behavior for any existing test since they're all well within one cycle.

Testing strategy deliberately avoids hand-crafting a byte-valid malicious file (a real decompression-bomb PoC — a tiny file with a header claiming huge dimensions — needs a structurally valid file format with a correct checksum/CRC just to reach the limits check, real effort for the same coverage): `image_decode_limits_reject_dimensions_beyond_16384`/`_allocations_beyond_128mb` test the actual `image::Limits` *values* wired up here directly against the crate's own `check_dimensions`/`reserve` methods (real, already-well-tested-upstream logic, not reimplemented) — proving the chosen numbers behave as intended without needing to drive a full decode.

Verified: `cargo test -p blitz-dom` — 75 passing (up from 72, +3) and `cargo test -p blitz-dom --features audio` — 81 (up from 78, +3, same three tests). Full outer desktop build and its 20-test suite both pass unchanged. Not independently re-verified against a real decompression-bomb file crafted by hand (deliberately, per the testing-strategy note above) or against a real, very-large legitimate image (e.g. a genuine 20-megapixel photo) to confirm the tightened limits don't false-positive-reject something a real page would legitimately serve — 16384px/128MB is comfortably above what a 20-megapixel photo needs (roughly 5000×4000, ~80MB uncompressed RGBA8), but an actual large real-world image hasn't been run through this path live.

---

### HTTP cache eviction (Phase 2.7)

The one piece of Phase 2.6's "deliberately not built this pass" list picked up next: `cacache` (the disk-cache backend both `net_cache.rs` and `blitz-net`'s own `cache` feature use) has no automatic size-based eviction at all — entries persist forever until manually cleared. Added `net_cache::evict_http_cache_over(max_bytes)`, called automatically (250MB default, `DEFAULT_HTTP_CACHE_LIMIT_BYTES`) every time `cached_client` builds a client.

**A real bug caught by the tests, not by inspection**: the first version trusted `cacache::Metadata.size` (from `list_sync`) to know each entry's on-disk size. Every test evicting against a computed total silently no-op'd — `size` read back as `0` for every entry. Root cause, found by checking `http-cache`'s own source (`http-cache-1.0.0-alpha.6/src/managers/cacache.rs`), not assumed: `Metadata.size` is only populated when the writer explicitly calls `cacache::WriteOpts::size(...)`; `CACacheManager::put` (the only real writer into this cache) writes via the plain `cacache::write()` helper, which never does. So this wasn't a test-only artifact — the real production cache would have had the exact same `size: 0` for every entry, making eviction a permanent no-op against real traffic too. Fixed by reading each entry's real content length via `cacache::read_sync` instead of trusting the index metadata.

A second real bug the same test run caught: the first version of the "removes oldest, keeps newest" test wrote identical `vec![0u8; 100]` content under three different keys — `cacache` is content-addressed, so all three collapsed onto one shared physical blob, and evicting the oldest key's content deleted it out from under the other two as well. Fixed by giving each test entry distinct bytes; a real reminder that `cacache`'s dedup is content-addressed, not key-addressed, when writing tests against it.

Eviction runs on a plain `std::thread::spawn`, not inline in `cached_client` and not `tokio::spawn`: `cached_client` sits on `Navigator::new()`'s path, which is on `himalayas daemon`'s startup path (`src/daemon/mod.rs`) — the same path this project measures and markets as ~30ms to ready (README's "The Proof"). A synchronous full-cache listing there on a long-lived cache would add real, user-visible startup latency. Plain `std::thread` (not `tokio::spawn`) so it works identically whether the caller already has a tokio runtime running (the daemon) or not (the native desktop shell's winit event loop, at the point `http_client()` first initializes its `OnceLock`).

Not built this pass, same as flagged in Phase 2.6: a bounded in-memory cache tier in front of the disk cache, and per-origin cache partitioning. "Oldest write wins" is also a real, stated limitation — `cacache`'s index only tracks a write timestamp, not a last-read one, so this is LRU-ish (evicts by insertion recency), not true LRU (by access recency); a genuinely-hot old entry gets evicted before a genuinely-cold recent one in a mixed workload. Fixing that would need a separate last-accessed tracking layer this pass didn't add.

Verified: `cargo test --lib net_cache` — 6 passing (up from 2, +4: no-op-when-missing, no-op-when-under-cap, removes-oldest-until-under-cap, removes-both-index-and-content). Full workspace: 255 lib + 172 bin + 4 outer doctests + 93 vendored-engine tests = 524, all passing, 2 `#[ignore]`d live-network tests not counted (the same `test_navigate_live_network`, present in both the lib and bin targets).
...
6. ✅ *(partial)* **Caching, the rest of it**: top-level document fetch and the `/app` `Navigator` path are cached (Phase 2.6) and the disk cache is now automatically size-capped with real eviction (Phase 2.7, see above). Still not built: a bounded in-memory cache tier in front of the disk cache (real browsers check memory before disk — though `blitz-net`'s own `image_cache` already covers the single highest-value case), true LRU (the eviction added is LRU-ish by write-time, not access-time — see Phase 2.7's note), and per-origin cache partitioning (matches the isolation-by-`Session` precedent `IsolationMode` already established for cookies).
...
11. **Adaptive Media Optimization Engine** (device/network/battery-aware quality selection for video/images, beyond the srcset/sizes work already done) — still blocked: the video half has nothing to adapt without `<video>` (not built — a real architecture decision on FFI-vs-pure-Rust decode, flagged in Phase 3, not decided yet).
12. **Display-configuration modes** ("Normal Laptop Mode" / "External Monitor Mode") — still not scoped past the original vision-doc reaction; needs a real pass over that doc's actual content, not something to guess at.
13. ✅ *(partial)* **Image security hardening** (decompression bombs) — done, see "Image decoder resource limits" above. **HDR/color management, Canvas/WebGL/WebGPU, AI/computer-vision image pipeline integration, decoder process sandboxing** — still not started; several of these (WebGPU texture upload, AI/CV pipeline efficiency) are specifically about *not* copying decoded video/image frames unnecessarily, which only means something once there's a frame pipeline (i.e. `<video>`) to avoid copying in.
